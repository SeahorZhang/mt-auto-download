  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        if (state.pagesProcessed >= CONFIG.SEARCH.MAX_PAGES_PER_SESSION) {
          logger.info(
            `[${state.searchType}：第${state.pageNumber}页] 已达到本次会话最大页数限制 (${CONFIG.SEARCH.MAX_PAGES_PER_SESSION})，程序结束`
          );
          break;
        }
