...
export class DownloadManager {
  ...
  async downloadTorrent(item, remainingCount) {
    ...
    try {
      ...
      logger.log(
        `[第${this.currentPage || '未知'}页] 准备下载: ${pc.cyan(item.name)}，大小: ${pc.bold(
  ...
  async uploadToQBittorrent(torrentPath, item) {
    try {
      logger.info(`[第${this.currentPage || '未知'}页] 📤 正在将种子上传到qBittorrent: ${item.name}`);
      ...
      if (success) {
        logger.success(`[第${this.currentPage || '未知'}页] ✅ 种子 "${item.name}" 已成功添加到qBittorrent`);
        logger.info(`[第${this.currentPage || '未知'}页] 📁 下载路径: ${CONFIG.QBITTORRENT.DOWNLOAD_PATH}`);
        logger.info(`[第${this.currentPage || '未知'}页] 📁 完成后将移动到: ${CONFIG.QBITTORRENT.FINAL_PATH}`);
        return true; // 返回成功状态
      } else {
        logger.error(`[第${this.currentPage || '未知'}页] ❌ 种子 "${item.name}" 添加到qBittorrent失败`);
        return false; // 返回失败状态
      }
    } catch (error) {
      logger.error(`[第${this.currentPage || '未知'}页] ❌ 上传种子到qBittorrent时发生错误: ${error.message}`);
      return false; // 返回失败状态
    }
  }
  ...
  async deleteTorrentFile(torrentPath, item) {
    try {
      ...
      if (fs.existsSync(torrentPath)) {
        ...
        logger.success(`[第${this.currentPage || '未知'}页] 🗑️ 已删除种子文件: ${path.basename(torrentPath)}`);
        ...
        if (files.length === 0) {
          ...
          logger.info(`[第${this.currentPage || '未知'}页] 📁 已删除空目录: ${torrentDir}`);
        }
      } else {
        logger.warn(`[第${this.currentPage || '未知'}页] ⚠️ 种子文件不存在，无法删除: ${torrentPath}`);
      }
    } catch (error) {
      logger.error(`[第${this.currentPage || '未知'}页] ❌ 删除种子文件失败: ${error.message}`);
    }
  }
